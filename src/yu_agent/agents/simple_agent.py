    ...
    def add_tool(self, tool) -> None:
        ...
        if hasattr(tool, 'auto_expand') and tool.auto_expand:
            ...
            if expanded_tools:
                ...
                print(f"   注册的工具名称: {', '.join([t.name for t in expanded_tools])}")
                return
            else:
                # 如果展开失败，打印诊断信息
                print(f"⚠️  警告：MCP工具 '{tool.name}' 展开失败，无可用工具")
                if hasattr(tool, '_available_tools'):
                    print(f"   可用工具列表: {tool._available_tools}")
