...
本模块实现情景/事件记忆（EpisodicMemory），负责记录发生过的具体交互事件。

设计要点：
- 时间序列组织：以时间为主轴组织记忆，支持回溯和时间范围检索。
- 权威存储：采用 SQLite 作为权威存储，Qdrant 用于向量索引/相似检索（若可用）。
- 会话/情景：支持 session_id 将若干事件组织为会话，便于获取会话内上下文历史。
- 回退策略：当向量检索不可用时会回退到本地缓存的关键词匹配。

实现说明：
- EpisodicMemory 同时维护内存缓存和权威数据库，读写以 SQLite 为准并在可用时同步向量索引。
- 检索过程常先做结构化过滤（时间/重要性），再做向量检索，并按相似度、近期性和重要性融合排序。
"""
