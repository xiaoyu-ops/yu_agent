...
本模块实现增强语义记忆（SemanticMemory），结合向量检索与知识图谱来保存与检索
概念性知识、实体与关系。适用于需要长期存储、复杂推理和关系查询的知识场景。

设计要点：
- 嵌入模型：使用统一的文本嵌入提供者（包装器），保证向量维度一致。
- 向量库：使用 Qdrant 作为向量检索后端以支持高效的相似度搜索。
- 图数据库：使用 Neo4j 存储实体/关系以支持基于图的推理与相关性计算。
- 混合检索：将向量相似度与图相关性融合，按重要性加权排序结果。

实现说明：
- 维护内存缓存（便于快速返回）以及将信息持久化到权威存储与向量库。
- 在检索时会并行进行向量与图检索，然后合并并去重，最后按混合分数排序返回。

注意：该模块依赖外部组件（Qdrant、Neo4j、HuggingFace 嵌入器）；在无依赖情况下会优雅降级。
"""
...
class SemanticMemory(BaseMemory):
    ...
    def get_all(self) -> List[MemoryItem]:
        """获取所有语义记忆"""
        return self.semantic_memories.copy() if self.semantic_memories else []
