...
本模块实现短期/工作记忆（WorkingMemory），用于存放会话级别的临时信息。

设计要点：
- 时效性：工作记忆通常只在短时间（如会话或数小时）内保持，使用 TTL 清理过期项。
- 容量限制：通过条目数量和 token 数量限制来避免内存膨胀。
- 优先级：基于记忆的重要性和时间衰减计算优先级，用于驱逐策略。
- 检索：优先尝试语义向量检索（TF-IDF 回退），再结合关键词匹配与时间衰减计算最终相关性得分。

实现说明：
- 内部存储为纯内存结构（列表 + 优先级堆），不要求持久化。
- 在添加时会维护 token 计数与堆结构；在检索时会进行过期清理。

注意：本实现使用 sklearn 的 TF-IDF 作为轻量向量检索示例，生产环境可替换为更强大的嵌入模型。
"""
...
class WorkingMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5, user_id: str = None, **kwargs) -> List[MemoryItem]:
        ...
        try:
            """
            此过程目的在于计算每条记忆与查询的相关性分数，结合向量相似度、关键词匹配和时间衰减来综合评估。
            1. 首先准备文档列表，包含查询和所有候选记忆的内容。
            2. 使用 TF-IDF 向量化文本，得到查询向量和记忆向量。
            3. 计算查询向量与每条记忆向量的余弦相似度，得到初步的相关性分数。
            """
            ...
            similarities = cosine_similarity(query_vector, doc_vectors).flatten() # 计算查询与每条记忆的相似度
        ...
        for memory in filtered_memories:
            ...
            # 时间衰减
            time_decay = self._calculate_time_decay(memory.timestamp) # 计算时间衰减因子
