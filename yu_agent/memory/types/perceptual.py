...
本模块实现感知记忆（PerceptualMemory），用于长期保存多模态感知数据（文本、图像、音频等）。

设计要点：
- 多模态存储：文本存 SQLite 为权威存储，图像/音频等可选使用向量存储（Qdrant）。
- 多模态索引：为每个模态维护独立的向量集合以避免维度冲突与查询干扰。
- 编码策略：优先使用 CLIP/CLAP 等模型进行跨模态编码，若不可用则使用确定性哈希或文本嵌入降级。
- 检索策略：同模态向量检索为主，结合时间与重要性对结果融合排序；无向量命中时回退到结构化检索与关键词匹配。

实现说明：
- PerceptualMemory 将感知对象封装为 `Perception`，并在 SQLite 中保存权威记录，向量化后写入 Qdrant。
- 支持跨模态检索接口 `cross_modal_search`（在具备 CLIP/CLAP 时效果最佳）。

注意：模块中有许多对外部依赖的兼容性处理，若缺少 CLIP/CLAP 或 Qdrant 会优雅退化为更简单的向量/哈希策略。
"""
